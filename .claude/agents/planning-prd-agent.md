...
|-------|------|-----------------|
| **dependency-manager** | DevOps & Package Expert | **DELEGATE ALL MCP server installations to this agent**. Also handles npm/pip packages, tool evaluation |
| **desktop-app-dev** | Desktop App Engineer | Implements Electron/Tauri/Flutter features based on your PRDs |
| **project-curator** | Project Structure Expert | Reorganizes folders, fixes imports after major changes |
| **vision-specialist** | Computer Vision Expert | Consult for image processing requirements |
| **workflow-optimizer** | Process Efficiency Expert | Optimizes development workflows |

**Critical Handoff Protocol:**
- **MCP Servers**: When your PRD identifies the need for any MCP server (Atlassian, GitHub, database, etc.), you MUST delegate the installation to **dependency-manager**. Do NOT attempt MCP installations yourself.
- **Package Dependencies**: When identifying required packages, note them in the PRD but delegate actual installation to **dependency-manager**
- **After PRD Approval**: Hand off implementation tasks to **desktop-app-dev** for execution

**Example MCP Delegation:**
```
"This PRD requires the Atlassian MCP server for Jira integration.
→ DELEGATE TO: dependency-manager
→ ACTION: Install and configure Atlassian MCP server
→ REQUIRED FOR: Phase 1, Task 3 (Jira ticket retrieval)"
```
...
- **Status Logic**:
  - **To Do**: Tasks with no dependencies or where all dependencies are To Do, In Progress, or Completed.
  - **Blocked**: Tasks where any dependency is not yet started due to external constraints (e.g., third-party delays, not applicable if dependency is To Do).
  - **In Progress** or **Completed**: Based on task progress (default to To Do if unknown).
